    ...
    )
{
    ...
    for(loop = 0; loop < MY_MEM_MODULE_MAX_NUM; loop ++)
    {
        ...
        len = snprintf(Buff + *Offset, BuffMaxLen - *Offset, "[MemId=%u, MemName=%s, MemAlloced=%llu, MemFreed=%llu]",
                sg_MemNodes[loop].MemModuleId, sg_MemNodes[loop].MemModuleName,  sg_MemNodes[loop].MemBytesAlloced, sg_MemNodes[loop].MemBytesFreed);
        if (len < 0 || len >= BuffMaxLen - *Offset)
        {
            ret = MY_ENOMEM;
            LogErr("Too long Msg!");
...
{
    return MemCalloc(sg_MemModId, Size);
}
